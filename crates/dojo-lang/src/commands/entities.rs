    ...
    fn from_ast(
        ...
        if entity_compontents.len() == 1 {
            ...
            for component in entity_compontents {
                variables.push(format!("__{}_ids_idxs", component.to_string().to_ascii_lowercase()));
                ...
                RewriteNode::interpolate_patched(
                    ...
                    loop {
                        match __$query_subtype$_ids_idxs.pop_front() {
                            Option::Some(idx) => {
                                let entity = serde::Serde::<$component$>::deserialize(__$query_id$_$query_subtype$_raw[*idx]).expect('$deser_err_msg$');
